...
class NetworkNotification {
  static const String _payload = 'Network';
  static const String _channel = 'Network';
  static const String _callCount = '[callCount]';

  /// Notification plugin instance
  FlutterLocalNotificationsPlugin? _flutterLocalNotificationsPlugin;

  /// Notification configuration for NetworkInspector.
  ...
  String _getNotificationMessage({
    ...
    required NetworkStats stats,
  }) => <String>[
    if (stats.loading > 0)
      '${context.i18n(NetworkTranslationKey.notificationLoading)} ${stats.loading}',
    if (stats.successes > 0)
      '${context.i18n(NetworkTranslationKey.notificationSuccess)} ${stats.successes}',
    if (stats.redirects > 0)
      '${context.i18n(NetworkTranslationKey.notificationRedirect)} ${stats.redirects}',
    if (stats.errors > 0)
      '${context.i18n(NetworkTranslationKey.notificationError)} ${stats.errors}',
  ].join(' | ');
  ...
  Future<void> showStatsNotification({
    ...
    try {
      ...
      await _flutterLocalNotificationsPlugin?.show(
        0,
        context
            .i18n(NetworkTranslationKey.notificationTotalRequests)
      ...
      NetworkUtils.log(error.toString());
    } finally {
