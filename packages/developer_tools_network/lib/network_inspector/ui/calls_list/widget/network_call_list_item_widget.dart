...
class NetworkCallListItemWidget extends StatelessWidget {
  const NetworkCallListItemWidget(this.call, this.itemClickAction, {super.key});

  final NetworkHttpCall call;
  final void Function(NetworkHttpCall) itemClickAction;
    ...
    return InkWell(
      ...
      child: Column(
        children: [
          ...
          const Divider(height: 1, color: NetworkTheme.grey),
        ],
      ),
    );
  }
  ...
  Color _getStatusTextColor(BuildContext context) => switch (call
    ...
    int status when status < 200 =>
      Theme.of(context).textTheme.bodyLarge?.color ?? NetworkTheme.grey,
    int status when status >= 200 && status < 300 => NetworkTheme.green,
    int status when status >= 300 && status < 400 => NetworkTheme.orange,
    int status when status >= 400 && status < 600 => NetworkTheme.red,
    _ => Theme.of(context).textTheme.bodyLarge!.color ?? NetworkTheme.grey,
  };

  /// Returns endpoint text color based on call state.
  Color _getEndpointTextColor(BuildContext context) =>
      call.loading ? NetworkTheme.grey : _getStatusTextColor(context);
}
      ...
      children: [
        Padding(
          ...
          child: Icon(
            ...
            color: call.secure ? NetworkTheme.green : NetworkTheme.red,
            size: 12,
          ),
        ),
        Expanded(
...
class _ResponseStatus extends StatelessWidget {
  ...
  String _getStatus(NetworkHttpResponse response) => switch (response.status) {
    -1 => 'ERR',
    0 => '???',
    _ => '${response.status}',
  };
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Flexible(
          child: Text(
            NetworkConversionHelper.formatTime(call.duration),
            style: const TextStyle(fontSize: 12),
          ),
        ),
        Flexible(
          child: Text(
            '${NetworkConversionHelper.formatBytes(call.request?.size ?? 0)} / '
            '${NetworkConversionHelper.formatBytes(call.response?.size ?? 0)}',
            style: const TextStyle(fontSize: 12),
          ),
        ),
      ],
